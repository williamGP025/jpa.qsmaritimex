    ...
    public void create(Port port) {
        ...
        try {
            ...
            for (Itinerary itineraryCollectionItinerary : port.getItineraryCollection()) {
                Port oldStartPortIdOfItineraryCollectionItinerary = itineraryCollectionItinerary.getStartPortId();
                itineraryCollectionItinerary.setStartPortId(port);
                itineraryCollectionItinerary = em.merge(itineraryCollectionItinerary);
                if (oldStartPortIdOfItineraryCollectionItinerary != null) {
                    oldStartPortIdOfItineraryCollectionItinerary.getItineraryCollection().remove(itineraryCollectionItinerary);
                    oldStartPortIdOfItineraryCollectionItinerary = em.merge(oldStartPortIdOfItineraryCollectionItinerary);
                }
            }
            for (Itinerary itineraryCollection1Itinerary : port.getItineraryCollection1()) {
                Port oldEndPortIdOfItineraryCollection1Itinerary = itineraryCollection1Itinerary.getEndPortId();
                itineraryCollection1Itinerary.setEndPortId(port);
                itineraryCollection1Itinerary = em.merge(itineraryCollection1Itinerary);
                if (oldEndPortIdOfItineraryCollection1Itinerary != null) {
                    oldEndPortIdOfItineraryCollection1Itinerary.getItineraryCollection1().remove(itineraryCollection1Itinerary);
                    oldEndPortIdOfItineraryCollection1Itinerary = em.merge(oldEndPortIdOfItineraryCollection1Itinerary);
                }
            }
            for (Itinerary itineraryCollection2Itinerary : port.getItineraryCollection2()) {
                Port oldOperatingPortIdOfItineraryCollection2Itinerary = itineraryCollection2Itinerary.getOperatingPortId();
                itineraryCollection2Itinerary.setOperatingPortId(port);
                itineraryCollection2Itinerary = em.merge(itineraryCollection2Itinerary);
                if (oldOperatingPortIdOfItineraryCollection2Itinerary != null) {
                    oldOperatingPortIdOfItineraryCollection2Itinerary.getItineraryCollection2().remove(itineraryCollection2Itinerary);
                    oldOperatingPortIdOfItineraryCollection2Itinerary = em.merge(oldOperatingPortIdOfItineraryCollection2Itinerary);
                }
            }
        ...
        try {
            ...
            for (Itinerary itineraryCollectionOldItinerary : itineraryCollectionOld) {
                if (!itineraryCollectionNew.contains(itineraryCollectionOldItinerary)) {
                    ...
                    illegalOrphanMessages.add("You must retain Itinerary " + itineraryCollectionOldItinerary + " since its startPortId field is not nullable.");
                }
            }
            for (Itinerary itineraryCollection1OldItinerary : itineraryCollection1Old) {
                if (!itineraryCollection1New.contains(itineraryCollection1OldItinerary)) {
                    ...
                    illegalOrphanMessages.add("You must retain Itinerary " + itineraryCollection1OldItinerary + " since its endPortId field is not nullable.");
                }
            }
            for (Itinerary itineraryCollection2OldItinerary : itineraryCollection2Old) {
                if (!itineraryCollection2New.contains(itineraryCollection2OldItinerary)) {
                    ...
                    illegalOrphanMessages.add("You must retain Itinerary " + itineraryCollection2OldItinerary + " since its operatingPortId field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Itinerary itineraryCollectionNewItinerary : itineraryCollectionNew) {
                if (!itineraryCollectionOld.contains(itineraryCollectionNewItinerary)) {
                    Port oldStartPortIdOfItineraryCollectionNewItinerary = itineraryCollectionNewItinerary.getStartPortId();
                    itineraryCollectionNewItinerary.setStartPortId(port);
                    itineraryCollectionNewItinerary = em.merge(itineraryCollectionNewItinerary);
                    if (oldStartPortIdOfItineraryCollectionNewItinerary != null && !oldStartPortIdOfItineraryCollectionNewItinerary.equals(port)) {
                        oldStartPortIdOfItineraryCollectionNewItinerary.getItineraryCollection().remove(itineraryCollectionNewItinerary);
                        oldStartPortIdOfItineraryCollectionNewItinerary = em.merge(oldStartPortIdOfItineraryCollectionNewItinerary);
                    }
                }
            }
            for (Itinerary itineraryCollection1NewItinerary : itineraryCollection1New) {
                if (!itineraryCollection1Old.contains(itineraryCollection1NewItinerary)) {
                    Port oldEndPortIdOfItineraryCollection1NewItinerary = itineraryCollection1NewItinerary.getEndPortId();
                    itineraryCollection1NewItinerary.setEndPortId(port);
                    itineraryCollection1NewItinerary = em.merge(itineraryCollection1NewItinerary);
                    if (oldEndPortIdOfItineraryCollection1NewItinerary != null && !oldEndPortIdOfItineraryCollection1NewItinerary.equals(port)) {
                        oldEndPortIdOfItineraryCollection1NewItinerary.getItineraryCollection1().remove(itineraryCollection1NewItinerary);
                        oldEndPortIdOfItineraryCollection1NewItinerary = em.merge(oldEndPortIdOfItineraryCollection1NewItinerary);
                    }
                }
            }
            for (Itinerary itineraryCollection2NewItinerary : itineraryCollection2New) {
                if (!itineraryCollection2Old.contains(itineraryCollection2NewItinerary)) {
                    Port oldOperatingPortIdOfItineraryCollection2NewItinerary = itineraryCollection2NewItinerary.getOperatingPortId();
                    itineraryCollection2NewItinerary.setOperatingPortId(port);
                    itineraryCollection2NewItinerary = em.merge(itineraryCollection2NewItinerary);
                    if (oldOperatingPortIdOfItineraryCollection2NewItinerary != null && !oldOperatingPortIdOfItineraryCollection2NewItinerary.equals(port)) {
                        oldOperatingPortIdOfItineraryCollection2NewItinerary.getItineraryCollection2().remove(itineraryCollection2NewItinerary);
                        oldOperatingPortIdOfItineraryCollection2NewItinerary = em.merge(oldOperatingPortIdOfItineraryCollection2NewItinerary);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            for (Itinerary itineraryCollectionOrphanCheckItinerary : itineraryCollectionOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Port (" + port + ") cannot be destroyed since the Itinerary " + itineraryCollectionOrphanCheckItinerary + " in its itineraryCollection field has a non-nullable startPortId field.");
            }
            ...
            for (Itinerary itineraryCollection1OrphanCheckItinerary : itineraryCollection1OrphanCheck) {
                ...
                illegalOrphanMessages.add("This Port (" + port + ") cannot be destroyed since the Itinerary " + itineraryCollection1OrphanCheckItinerary + " in its itineraryCollection1 field has a non-nullable endPortId field.");
            }
            ...
            for (Itinerary itineraryCollection2OrphanCheckItinerary : itineraryCollection2OrphanCheck) {
                ...
                illegalOrphanMessages.add("This Port (" + port + ") cannot be destroyed since the Itinerary " + itineraryCollection2OrphanCheckItinerary + " in its itineraryCollection2 field has a non-nullable operatingPortId field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
